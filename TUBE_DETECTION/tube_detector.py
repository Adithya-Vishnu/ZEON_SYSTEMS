...
# ============================================================
#  STEP 1 — DETECT TUBE POSITIONS (Hough Circle Transform)
# ============================================================
...
# ============================================================
#  STEP 2 — ESTIMATE TUBE ANGLE
# ============================================================

def estimate_angle(image_path, cx, cy, radius):
    """
    Estimates the angle of the tube lid tab (joint to tab direction).
    Samples brightness in a ring around the circle; brightest spot = tab.
    Returns angle in degrees [0, 360), counter-clockwise from positive X.
    """
    img  = cv2.imread(image_path)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    h, w = gray.shape
...
# ============================================================
#  STEP 3 — RUN ON ALL IMAGES
# ============================================================

def run_detection(images_folder, annotations_csv):
    """
    Loops over every image in annotations.csv, runs detection, returns results DataFrame.
    """
...
def evaluate(predictions_df, annotations_csv):
    ...
    DISTANCE_THRESHOLD = 20  # pixels — how close a prediction must be to count as a match
    ...
    for img_name in df_gt["image"].unique():
        ...
        for gx, gy, ga in gt_list:
            ...
            if best_pi >= 0 and best_dist <= DISTANCE_THRESHOLD:
                # matched — record position and angle errors
